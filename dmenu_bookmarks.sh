...
URL_FILE_PATH="$HOME/.local/configs/dotfiles/"
...
bmks_display() {
	bmks_check
	case $PREFERED_LAUNCHER in
		dmenu) cat $URL_FILE_PATH/$URL_FILE_NAME | dmenu -i -p "BOOKMARK" -l 10 | awk '{print $(NF)}' | xargs -I '{}' $BROWSER {} ;;
...
case "$1" in
    ...
    "-o")  BROWSER='firefox --new-window';bmks_display;;
